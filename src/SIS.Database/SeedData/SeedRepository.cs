    ...
    public class SeedRepository : ISeedRepository
    {
        ...
        public void SeedUsers()
        {
            if (!_userManager.Users.Any())
            {
                ...
                IdentityResult guestResult = _userManager.CreateAsync(guest, "guest").Result;
                #endregion
                ...
                {
                    var userApplicant = _userManager.FindByNameAsync("user").Result;
                    _userManager.AddToRolesAsync(userApplicant, new[] { "User" }).Wait();
                }
                if (guestResult.Succeeded)
                {
                    var guestApplicant = _userManager.FindByNameAsync("guest").Result;
                    _userManager.AddToRolesAsync(guestApplicant, new[] { "Guest" }).Wait();
                }
            }
        }
    }
